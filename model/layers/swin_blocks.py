...
class swin_encoder_block(nn.Module):
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
        ...
        self.attention = window_self_attention(
    ...
    def forward(self, x, mask_matrix):
        ...
        # W-MSA/SW-MSA
        attn_windows = self.attention(x_windows, mask=attention_mask)  # nW*B, window_size*window_size, C
...
class swin_encoder_block_withoutPos(nn.Module):
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, inverse=False):
        super().__init__()
        ...
        self.norm1 = norm_layer(dim)
        self.attention = window_self_attention_without_pos(
            dim, window_size=to_2tuple(self.window_size), num_heads=num_heads,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
